// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        (unknown)
// source: ar/v1/environment.proto

package arv1

import (
	_ "buf.build/gen/go/bufbuild/protovalidate/protocolbuffers/go/buf/validate"
	v1 "github.com/cobotar/protocol/messages/geometry/v1"
	_ "github.com/cobotar/protocol/messages/validation/v1"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type EnvironmentType int32

const (
	EnvironmentType_ENVIRONMENT_TYPE_UNSPECIFIED       EnvironmentType = 0
	EnvironmentType_ENVIRONMENT_TYPE_STORAGE           EnvironmentType = 1
	EnvironmentType_ENVIRONMENT_TYPE_MANUAL_STATION    EnvironmentType = 2
	EnvironmentType_ENVIRONMENT_TYPE_AUTOMATIC_STATION EnvironmentType = 3
	EnvironmentType_ENVIRONMENT_TYPE_HYBRID_STATION    EnvironmentType = 4
	EnvironmentType_ENVIRONMENT_TYPE_MANUAL_LINE       EnvironmentType = 5
	EnvironmentType_ENVIRONMENT_TYPE_AUTOMATIC_LINE    EnvironmentType = 6
	EnvironmentType_ENVIRONMENT_TYPE_HYBRID_LINE       EnvironmentType = 7
)

// Enum value maps for EnvironmentType.
var (
	EnvironmentType_name = map[int32]string{
		0: "ENVIRONMENT_TYPE_UNSPECIFIED",
		1: "ENVIRONMENT_TYPE_STORAGE",
		2: "ENVIRONMENT_TYPE_MANUAL_STATION",
		3: "ENVIRONMENT_TYPE_AUTOMATIC_STATION",
		4: "ENVIRONMENT_TYPE_HYBRID_STATION",
		5: "ENVIRONMENT_TYPE_MANUAL_LINE",
		6: "ENVIRONMENT_TYPE_AUTOMATIC_LINE",
		7: "ENVIRONMENT_TYPE_HYBRID_LINE",
	}
	EnvironmentType_value = map[string]int32{
		"ENVIRONMENT_TYPE_UNSPECIFIED":       0,
		"ENVIRONMENT_TYPE_STORAGE":           1,
		"ENVIRONMENT_TYPE_MANUAL_STATION":    2,
		"ENVIRONMENT_TYPE_AUTOMATIC_STATION": 3,
		"ENVIRONMENT_TYPE_HYBRID_STATION":    4,
		"ENVIRONMENT_TYPE_MANUAL_LINE":       5,
		"ENVIRONMENT_TYPE_AUTOMATIC_LINE":    6,
		"ENVIRONMENT_TYPE_HYBRID_LINE":       7,
	}
)

func (x EnvironmentType) Enum() *EnvironmentType {
	p := new(EnvironmentType)
	*p = x
	return p
}

func (x EnvironmentType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (EnvironmentType) Descriptor() protoreflect.EnumDescriptor {
	return file_ar_v1_environment_proto_enumTypes[0].Descriptor()
}

func (EnvironmentType) Type() protoreflect.EnumType {
	return &file_ar_v1_environment_proto_enumTypes[0]
}

func (x EnvironmentType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use EnvironmentType.Descriptor instead.
func (EnvironmentType) EnumDescriptor() ([]byte, []int) {
	return file_ar_v1_environment_proto_rawDescGZIP(), []int{0}
}

type MarkerLocation struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Location      *v1.LocalizedPose      `protobuf:"bytes,2,opt,name=location,proto3" json:"location,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MarkerLocation) Reset() {
	*x = MarkerLocation{}
	mi := &file_ar_v1_environment_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MarkerLocation) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MarkerLocation) ProtoMessage() {}

func (x *MarkerLocation) ProtoReflect() protoreflect.Message {
	mi := &file_ar_v1_environment_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MarkerLocation.ProtoReflect.Descriptor instead.
func (*MarkerLocation) Descriptor() ([]byte, []int) {
	return file_ar_v1_environment_proto_rawDescGZIP(), []int{0}
}

func (x *MarkerLocation) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *MarkerLocation) GetLocation() *v1.LocalizedPose {
	if x != nil {
		return x.Location
	}
	return nil
}

type RobotLocation struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Location      *v1.LocalizedPose      `protobuf:"bytes,2,opt,name=location,proto3" json:"location,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RobotLocation) Reset() {
	*x = RobotLocation{}
	mi := &file_ar_v1_environment_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RobotLocation) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RobotLocation) ProtoMessage() {}

func (x *RobotLocation) ProtoReflect() protoreflect.Message {
	mi := &file_ar_v1_environment_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RobotLocation.ProtoReflect.Descriptor instead.
func (*RobotLocation) Descriptor() ([]byte, []int) {
	return file_ar_v1_environment_proto_rawDescGZIP(), []int{1}
}

func (x *RobotLocation) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *RobotLocation) GetLocation() *v1.LocalizedPose {
	if x != nil {
		return x.Location
	}
	return nil
}

type AssetLocation struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Location      *v1.LocalizedPose      `protobuf:"bytes,2,opt,name=location,proto3" json:"location,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AssetLocation) Reset() {
	*x = AssetLocation{}
	mi := &file_ar_v1_environment_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AssetLocation) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AssetLocation) ProtoMessage() {}

func (x *AssetLocation) ProtoReflect() protoreflect.Message {
	mi := &file_ar_v1_environment_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AssetLocation.ProtoReflect.Descriptor instead.
func (*AssetLocation) Descriptor() ([]byte, []int) {
	return file_ar_v1_environment_proto_rawDescGZIP(), []int{2}
}

func (x *AssetLocation) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *AssetLocation) GetLocation() *v1.LocalizedPose {
	if x != nil {
		return x.Location
	}
	return nil
}

type PartLocation struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Location      *v1.LocalizedPose      `protobuf:"bytes,2,opt,name=location,proto3" json:"location,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PartLocation) Reset() {
	*x = PartLocation{}
	mi := &file_ar_v1_environment_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PartLocation) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PartLocation) ProtoMessage() {}

func (x *PartLocation) ProtoReflect() protoreflect.Message {
	mi := &file_ar_v1_environment_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PartLocation.ProtoReflect.Descriptor instead.
func (*PartLocation) Descriptor() ([]byte, []int) {
	return file_ar_v1_environment_proto_rawDescGZIP(), []int{3}
}

func (x *PartLocation) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *PartLocation) GetLocation() *v1.LocalizedPose {
	if x != nil {
		return x.Location
	}
	return nil
}

type ToolLocation struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Location      *v1.LocalizedPose      `protobuf:"bytes,2,opt,name=location,proto3" json:"location,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ToolLocation) Reset() {
	*x = ToolLocation{}
	mi := &file_ar_v1_environment_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ToolLocation) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ToolLocation) ProtoMessage() {}

func (x *ToolLocation) ProtoReflect() protoreflect.Message {
	mi := &file_ar_v1_environment_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ToolLocation.ProtoReflect.Descriptor instead.
func (*ToolLocation) Descriptor() ([]byte, []int) {
	return file_ar_v1_environment_proto_rawDescGZIP(), []int{4}
}

func (x *ToolLocation) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *ToolLocation) GetLocation() *v1.LocalizedPose {
	if x != nil {
		return x.Location
	}
	return nil
}

type FixtureLocation struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Location      *v1.LocalizedPose      `protobuf:"bytes,2,opt,name=location,proto3" json:"location,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *FixtureLocation) Reset() {
	*x = FixtureLocation{}
	mi := &file_ar_v1_environment_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FixtureLocation) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FixtureLocation) ProtoMessage() {}

func (x *FixtureLocation) ProtoReflect() protoreflect.Message {
	mi := &file_ar_v1_environment_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FixtureLocation.ProtoReflect.Descriptor instead.
func (*FixtureLocation) Descriptor() ([]byte, []int) {
	return file_ar_v1_environment_proto_rawDescGZIP(), []int{5}
}

func (x *FixtureLocation) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *FixtureLocation) GetLocation() *v1.LocalizedPose {
	if x != nil {
		return x.Location
	}
	return nil
}

type EnvironmentMessage struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Name          string                 `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	Icon          string                 `protobuf:"bytes,3,opt,name=icon,proto3" json:"icon,omitempty"`
	Description   string                 `protobuf:"bytes,4,opt,name=description,proto3" json:"description,omitempty"`
	Type          EnvironmentType        `protobuf:"varint,5,opt,name=type,proto3,enum=ar.v1.EnvironmentType" json:"type,omitempty"` // Type of environment
	Markers       []*MarkerLocation      `protobuf:"bytes,6,rep,name=markers,proto3" json:"markers,omitempty"`                       // Markers associated with this environment.
	Robots        []*RobotLocation       `protobuf:"bytes,7,rep,name=robots,proto3" json:"robots,omitempty"`                         // Robot located in this environment
	Assets        []*AssetLocation       `protobuf:"bytes,8,rep,name=assets,proto3" json:"assets,omitempty"`                         // Assets located in this environment
	Parts         []*PartLocation        `protobuf:"bytes,9,rep,name=parts,proto3" json:"parts,omitempty"`                           // Parts located in this environment
	Tools         []*ToolLocation        `protobuf:"bytes,10,rep,name=tools,proto3" json:"tools,omitempty"`                          // Tools located in this environment
	Fixtures      []*FixtureLocation     `protobuf:"bytes,11,rep,name=fixtures,proto3" json:"fixtures,omitempty"`                    // Fixtures located in this environment
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EnvironmentMessage) Reset() {
	*x = EnvironmentMessage{}
	mi := &file_ar_v1_environment_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EnvironmentMessage) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EnvironmentMessage) ProtoMessage() {}

func (x *EnvironmentMessage) ProtoReflect() protoreflect.Message {
	mi := &file_ar_v1_environment_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EnvironmentMessage.ProtoReflect.Descriptor instead.
func (*EnvironmentMessage) Descriptor() ([]byte, []int) {
	return file_ar_v1_environment_proto_rawDescGZIP(), []int{6}
}

func (x *EnvironmentMessage) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *EnvironmentMessage) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *EnvironmentMessage) GetIcon() string {
	if x != nil {
		return x.Icon
	}
	return ""
}

func (x *EnvironmentMessage) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *EnvironmentMessage) GetType() EnvironmentType {
	if x != nil {
		return x.Type
	}
	return EnvironmentType_ENVIRONMENT_TYPE_UNSPECIFIED
}

func (x *EnvironmentMessage) GetMarkers() []*MarkerLocation {
	if x != nil {
		return x.Markers
	}
	return nil
}

func (x *EnvironmentMessage) GetRobots() []*RobotLocation {
	if x != nil {
		return x.Robots
	}
	return nil
}

func (x *EnvironmentMessage) GetAssets() []*AssetLocation {
	if x != nil {
		return x.Assets
	}
	return nil
}

func (x *EnvironmentMessage) GetParts() []*PartLocation {
	if x != nil {
		return x.Parts
	}
	return nil
}

func (x *EnvironmentMessage) GetTools() []*ToolLocation {
	if x != nil {
		return x.Tools
	}
	return nil
}

func (x *EnvironmentMessage) GetFixtures() []*FixtureLocation {
	if x != nil {
		return x.Fixtures
	}
	return nil
}

type EnvironmentMessages struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Environments  []*EnvironmentMessage  `protobuf:"bytes,1,rep,name=environments,proto3" json:"environments,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EnvironmentMessages) Reset() {
	*x = EnvironmentMessages{}
	mi := &file_ar_v1_environment_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EnvironmentMessages) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EnvironmentMessages) ProtoMessage() {}

func (x *EnvironmentMessages) ProtoReflect() protoreflect.Message {
	mi := &file_ar_v1_environment_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EnvironmentMessages.ProtoReflect.Descriptor instead.
func (*EnvironmentMessages) Descriptor() ([]byte, []int) {
	return file_ar_v1_environment_proto_rawDescGZIP(), []int{7}
}

func (x *EnvironmentMessages) GetEnvironments() []*EnvironmentMessage {
	if x != nil {
		return x.Environments
	}
	return nil
}

var File_ar_v1_environment_proto protoreflect.FileDescriptor

const file_ar_v1_environment_proto_rawDesc = "" +
	"\n" +
	"\x17ar/v1/environment.proto\x12\x05ar.v1\x1a\x1bbuf/validate/validate.proto\x1a\x16geometry/v1/pose.proto\x1a+validation/v1/predefined_string_rules.proto\"k\n" +
	"\x0eMarkerLocation\x12\x19\n" +
	"\x02id\x18\x01 \x01(\tB\t\xbaH\x06r\x04\xe8\xf1\x04\x01R\x02id\x12>\n" +
	"\blocation\x18\x02 \x01(\v2\x1a.geometry.v1.LocalizedPoseB\x06\xbaH\x03\xc8\x01\x01R\blocation\"j\n" +
	"\rRobotLocation\x12\x19\n" +
	"\x02id\x18\x01 \x01(\tB\t\xbaH\x06r\x04\xa0\xf1\x04\x01R\x02id\x12>\n" +
	"\blocation\x18\x02 \x01(\v2\x1a.geometry.v1.LocalizedPoseB\x06\xbaH\x03\xc8\x01\x01R\blocation\"j\n" +
	"\rAssetLocation\x12\x19\n" +
	"\x02id\x18\x01 \x01(\tB\t\xbaH\x06r\x04\xb0\xf1\x04\x01R\x02id\x12>\n" +
	"\blocation\x18\x02 \x01(\v2\x1a.geometry.v1.LocalizedPoseB\x06\xbaH\x03\xc8\x01\x01R\blocation\"i\n" +
	"\fPartLocation\x12\x19\n" +
	"\x02id\x18\x01 \x01(\tB\t\xbaH\x06r\x04\xc8\xf1\x04\x01R\x02id\x12>\n" +
	"\blocation\x18\x02 \x01(\v2\x1a.geometry.v1.LocalizedPoseB\x06\xbaH\x03\xc8\x01\x01R\blocation\"i\n" +
	"\fToolLocation\x12\x19\n" +
	"\x02id\x18\x01 \x01(\tB\t\xbaH\x06r\x04\xd8\xf1\x04\x01R\x02id\x12>\n" +
	"\blocation\x18\x02 \x01(\v2\x1a.geometry.v1.LocalizedPoseB\x06\xbaH\x03\xc8\x01\x01R\blocation\"l\n" +
	"\x0fFixtureLocation\x12\x19\n" +
	"\x02id\x18\x01 \x01(\tB\t\xbaH\x06r\x04\xf0\xf1\x04\x01R\x02id\x12>\n" +
	"\blocation\x18\x02 \x01(\v2\x1a.geometry.v1.LocalizedPoseB\x06\xbaH\x03\xc8\x01\x01R\blocation\"\xd5\x03\n" +
	"\x12EnvironmentMessage\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x1d\n" +
	"\x04name\x18\x02 \x01(\tB\t\xbaH\x06r\x04\x80\xf1\x04\x01R\x04name\x12\x12\n" +
	"\x04icon\x18\x03 \x01(\tR\x04icon\x12 \n" +
	"\vdescription\x18\x04 \x01(\tR\vdescription\x129\n" +
	"\x04type\x18\x05 \x01(\x0e2\x16.ar.v1.EnvironmentTypeB\r\xbaH\n" +
	"\xc8\x01\x01\x82\x01\x04\x10\x01(\x04R\x04type\x129\n" +
	"\amarkers\x18\x06 \x03(\v2\x15.ar.v1.MarkerLocationB\b\xbaH\x05\x92\x01\x02\b\x01R\amarkers\x12,\n" +
	"\x06robots\x18\a \x03(\v2\x14.ar.v1.RobotLocationR\x06robots\x12,\n" +
	"\x06assets\x18\b \x03(\v2\x14.ar.v1.AssetLocationR\x06assets\x12)\n" +
	"\x05parts\x18\t \x03(\v2\x13.ar.v1.PartLocationR\x05parts\x12)\n" +
	"\x05tools\x18\n" +
	" \x03(\v2\x13.ar.v1.ToolLocationR\x05tools\x122\n" +
	"\bfixtures\x18\v \x03(\v2\x16.ar.v1.FixtureLocationR\bfixtures\"T\n" +
	"\x13EnvironmentMessages\x12=\n" +
	"\fenvironments\x18\x01 \x03(\v2\x19.ar.v1.EnvironmentMessageR\fenvironments*\xac\x02\n" +
	"\x0fEnvironmentType\x12 \n" +
	"\x1cENVIRONMENT_TYPE_UNSPECIFIED\x10\x00\x12\x1c\n" +
	"\x18ENVIRONMENT_TYPE_STORAGE\x10\x01\x12#\n" +
	"\x1fENVIRONMENT_TYPE_MANUAL_STATION\x10\x02\x12&\n" +
	"\"ENVIRONMENT_TYPE_AUTOMATIC_STATION\x10\x03\x12#\n" +
	"\x1fENVIRONMENT_TYPE_HYBRID_STATION\x10\x04\x12 \n" +
	"\x1cENVIRONMENT_TYPE_MANUAL_LINE\x10\x05\x12#\n" +
	"\x1fENVIRONMENT_TYPE_AUTOMATIC_LINE\x10\x06\x12 \n" +
	"\x1cENVIRONMENT_TYPE_HYBRID_LINE\x10\aB\x8c\x01\n" +
	"\tcom.ar.v1B\x10EnvironmentProtoP\x01Z/github.com/cobotar/protocol/messages/ar/v1;arv1\xa2\x02\x03AXX\xaa\x02\x0eMessages.AR.V1\xca\x02\x05Ar\\V1\xe2\x02\x11Ar\\V1\\GPBMetadata\xea\x02\x06Ar::V1b\x06proto3"

var (
	file_ar_v1_environment_proto_rawDescOnce sync.Once
	file_ar_v1_environment_proto_rawDescData []byte
)

func file_ar_v1_environment_proto_rawDescGZIP() []byte {
	file_ar_v1_environment_proto_rawDescOnce.Do(func() {
		file_ar_v1_environment_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_ar_v1_environment_proto_rawDesc), len(file_ar_v1_environment_proto_rawDesc)))
	})
	return file_ar_v1_environment_proto_rawDescData
}

var file_ar_v1_environment_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_ar_v1_environment_proto_msgTypes = make([]protoimpl.MessageInfo, 8)
var file_ar_v1_environment_proto_goTypes = []any{
	(EnvironmentType)(0),        // 0: ar.v1.EnvironmentType
	(*MarkerLocation)(nil),      // 1: ar.v1.MarkerLocation
	(*RobotLocation)(nil),       // 2: ar.v1.RobotLocation
	(*AssetLocation)(nil),       // 3: ar.v1.AssetLocation
	(*PartLocation)(nil),        // 4: ar.v1.PartLocation
	(*ToolLocation)(nil),        // 5: ar.v1.ToolLocation
	(*FixtureLocation)(nil),     // 6: ar.v1.FixtureLocation
	(*EnvironmentMessage)(nil),  // 7: ar.v1.EnvironmentMessage
	(*EnvironmentMessages)(nil), // 8: ar.v1.EnvironmentMessages
	(*v1.LocalizedPose)(nil),    // 9: geometry.v1.LocalizedPose
}
var file_ar_v1_environment_proto_depIdxs = []int32{
	9,  // 0: ar.v1.MarkerLocation.location:type_name -> geometry.v1.LocalizedPose
	9,  // 1: ar.v1.RobotLocation.location:type_name -> geometry.v1.LocalizedPose
	9,  // 2: ar.v1.AssetLocation.location:type_name -> geometry.v1.LocalizedPose
	9,  // 3: ar.v1.PartLocation.location:type_name -> geometry.v1.LocalizedPose
	9,  // 4: ar.v1.ToolLocation.location:type_name -> geometry.v1.LocalizedPose
	9,  // 5: ar.v1.FixtureLocation.location:type_name -> geometry.v1.LocalizedPose
	0,  // 6: ar.v1.EnvironmentMessage.type:type_name -> ar.v1.EnvironmentType
	1,  // 7: ar.v1.EnvironmentMessage.markers:type_name -> ar.v1.MarkerLocation
	2,  // 8: ar.v1.EnvironmentMessage.robots:type_name -> ar.v1.RobotLocation
	3,  // 9: ar.v1.EnvironmentMessage.assets:type_name -> ar.v1.AssetLocation
	4,  // 10: ar.v1.EnvironmentMessage.parts:type_name -> ar.v1.PartLocation
	5,  // 11: ar.v1.EnvironmentMessage.tools:type_name -> ar.v1.ToolLocation
	6,  // 12: ar.v1.EnvironmentMessage.fixtures:type_name -> ar.v1.FixtureLocation
	7,  // 13: ar.v1.EnvironmentMessages.environments:type_name -> ar.v1.EnvironmentMessage
	14, // [14:14] is the sub-list for method output_type
	14, // [14:14] is the sub-list for method input_type
	14, // [14:14] is the sub-list for extension type_name
	14, // [14:14] is the sub-list for extension extendee
	0,  // [0:14] is the sub-list for field type_name
}

func init() { file_ar_v1_environment_proto_init() }
func file_ar_v1_environment_proto_init() {
	if File_ar_v1_environment_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_ar_v1_environment_proto_rawDesc), len(file_ar_v1_environment_proto_rawDesc)),
			NumEnums:      1,
			NumMessages:   8,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_ar_v1_environment_proto_goTypes,
		DependencyIndexes: file_ar_v1_environment_proto_depIdxs,
		EnumInfos:         file_ar_v1_environment_proto_enumTypes,
		MessageInfos:      file_ar_v1_environment_proto_msgTypes,
	}.Build()
	File_ar_v1_environment_proto = out.File
	file_ar_v1_environment_proto_goTypes = nil
	file_ar_v1_environment_proto_depIdxs = nil
}
